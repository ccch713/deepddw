...
function gw(ctx) {
	return window.location.origin || "http://127.0.0.1:8600";
}
function t(ctx, k) {
...
function tryAutoIdentify(ctx, baseUrl) {
	...
	if (localStorage.getItem("deepddw_member_id")) return;
	fetch(baseUrl + "/api/v1/member/list").then((r) => r.json()).then((d) => {
		...
		}, () => (0, react.createElement)(MemberIdentify, {
			members,
			deviceId,
			baseUrl
		})));
...
function setupOnboarding(ctx) {
	...
	fetch(baseUrl + "/api/v1/deployment/mode").then((r) => r.json()).then((d) => {
		...
		}, () => (0, react.createElement)(OnboardingModal, {
			baseUrl,
			ctx
		})));
	}).catch(() => {});
...
function setupSection(ctx) {
	ctx.slots.inject("settings.section", () => ctx.slots.register({
		...
		order: 100,
		label: () => "多用户设置"
	}, () => (0, react.createElement)(SettingsPanel, { ctx })));
}
...
const OnboardingModal = {
	...
	methods: { async confirm() {
		this.submitting = true;
		try {
			if ((await fetch(this.baseUrl + "/api/v1/deployment/mode", {
				method: "POST",
	...
	render() {
		return (0, react.createElement)("div", { style: "position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.5)" }, [(0, react.createElement)("div", { style: "background:var(--k-color-bg,#1a1a2e);border-radius:12px;padding:32px;max-width:420px;width:90%;color:var(--k-color-text,#e8ecf8)" }, [
			(0, react.createElement)("h2", { style: "margin:0 0 8px;font-size:18px;font-weight:700" }, "选择使用模式"),
			(0, react.createElement)("p", { style: "margin:0 0 20px;font-size:13px;color:var(--k-color-text-3,#888)" }, "可随时在「设置 → 多用户设置」中切换"),
			...MODES.map((m) => (0, react.createElement)("label", {
				style: "display:flex;align-items:center;gap:10px;padding:12px 14px;margin-bottom:8px;border-radius:8px;cursor:pointer;border:2px solid " + (this.selected === m.value ? "var(--k-color-primary,#00e5ff)" : "transparent") + ";background:" + (this.selected === m.value ? "var(--k-color-primary-bg,#00e5ff10)" : "transparent"),
				onClick: () => {
			...
			}), (0, react.createElement)("div", null, [(0, react.createElement)("div", { style: "font-size:14px;font-weight:600" }, m.label), (0, react.createElement)("div", { style: "font-size:12px;color:var(--k-color-text-3,#888)" }, m.desc)])])),
...
const SettingsPanel = {
	...
	methods: {
		...
		async setMode(m) {
			const b = gw(this.ctx);
			if ((await fetch(b + "/api/v1/deployment/mode", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
	...
	render() {
		...
		return h2("div", { style: "padding:16px;color:var(--k-color-text,#e8ecf8)" }, [
			...
			h2("p", { style: "font-size:12px;color:var(--k-color-text-3,#888);margin:0 0 20px" }, "管理多台设备、多名成员的共享与隔离"),
			h2("div", { style: "font-size:13px;font-weight:600;margin-bottom:10px;color:var(--k-color-text-2,#ccc)" }, "模式"),
			...MODES.map((m) => h2("label", {
				key: m.value,
				style: "display:flex;align-items:center;gap:10px;padding:10px 14px;margin-bottom:6px;border-radius:8px;cursor:pointer;border:2px solid " + (this.mode === m.value ? "var(--k-color-primary,#00e5ff)" : "transparent")
			}, [h2("input", {
				type: "radio",
				checked: this.mode === m.value,
				onChange: () => this.setMode(m.value),
				style: "accent-color:var(--k-color-primary,#00e5ff)"
			...
			h2("div", { style: "display:flex;gap:8px;margin-bottom:12px" }, [h2("input", {
				value: this.newName,
				onInput: (e) => {
			...
			this.members.length === 0 ? h2("div", { style: "font-size:12px;color:var(--k-color-text-3,#888);padding:8px 0" }, "暂无成员，点击 + 添加") : this.members.map((m) => h2("div", {
				key: m.member_id,
				...
				h2("button", {
					onClick: () => this.removeMember(m.member_id),
					style: "padding:4px 8px;border:1px solid var(--k-color-border,#444);border-radius:4px;background:transparent;color:var(--k-color-text-3,#888);font-size:11px;cursor:pointer"
				}, "移除")
			])),
...
const MemberIdentify = {
	...
	render() {
		...
		return (0, react.createElement)("div", { style: "position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.5)" }, [(0, react.createElement)("div", { style: "background:var(--k-color-bg,#1a1a2e);border-radius:12px;padding:32px;max-width:380px;width:90%;color:var(--k-color-text,#e8ecf8)" }, [
			(0, react.createElement)("h2", { style: "margin:0 0 16px;font-size:17px;font-weight:700" }, "你是谁？"),
			...this.members.map((m) => (0, react.createElement)("label", {
				key: m.member_id,
				onClick: () => {
					this.selected = m.member_id;
				},
				style: "display:flex;align-items:center;gap:10px;padding:12px 14px;margin-bottom:6px;border-radius:8px;cursor:pointer;border:2px solid " + (this.selected === m.member_id ? "var(--k-color-primary,#00e5ff)" : "transparent")
			}, [(0, react.createElement)("input", {
				type: "radio",
				checked: this.selected === m.member_id,
				style: "accent-color:var(--k-color-primary,#00e5ff)"
			}), (0, react.createElement)("span", { style: "font-size:14px" }, m.display_name)])),
			(0, react.createElement)("button", {
				onClick: this.bind,
