...
def _seed_paid(client, amount=500.0):
    payload = {
        ...
        "doctor_id": "doc_001",
        "items": [{"item_name": "x", "unit_price": amount, "subtotal": amount, "quantity": 1, "treatment_type": "extraction"}],
...
def test_T15_5_multi_channel_reconcile(client, setup):
    ...
    for rid, ch in [(rid1, "wechat_pay"), (rid2, "alipay")]:
        r = client.post(
            "/api/v1/plugins/ddw_aggregated_pay/transactions",
            json={"payment_record_id": rid, "channel": ch, "amount": 300.0 if ch == "wechat_pay" else 400.0},
        )
        tid = r.json()["id"]
        client.patch(
            f"/api/v1/plugins/ddw_aggregated_pay/transactions/{tid}",
