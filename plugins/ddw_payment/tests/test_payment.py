...
import pytest
from ddw_payment import router as plugin_router
...
def _payload(**over):
    base = {
        ...
        "doctor_id": "doc_001",
        "items": [
            {"item_name": "拔牙", "quantity": 1, "unit_price": 500.0,
                "subtotal": 500.0, "treatment_type": "extraction"},
...
def test_T5_1_create_record_total_correct(client):
    resp = client.post("/api/v1/plugins/ddw_payment/records", json=_payload())
...
def test_T5_2_mark_paid(client):
    rid = client.post(
        "/api/v1/plugins/ddw_payment/records", json=_payload()
    ).json()["id"]
    resp = client.post(f"/api/v1/plugins/ddw_payment/records/{rid}/pay")
...
def test_T5_3_refund(client):
    ...
    resp = client.post(
        f"/api/v1/plugins/ddw_payment/records/{rid}/refund",
...
def test_T5_4_daily_summary_by_method(client):
    ...
    for m in ("wechat", "alipay", "cash"):
        rid = client.post(
            "/api/v1/plugins/ddw_payment/records",
            json=_payload(payment_method=m),
        ).json()["id"]
        client.post(f"/api/v1/plugins/ddw_payment/records/{rid}/pay")
    resp = client.get(
        "/api/v1/plugins/ddw_payment/daily-summary",
        params={"date": today},
    )
    body = resp.json()
...
def test_T5_5_filter_by_date(client):
    client.post("/api/v1/plugins/ddw_payment/records", json=_payload())
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    resp = client.get(
        "/api/v1/plugins/ddw_payment/records",
        params={"date": today},
...
def test_T5_6_receipt_number_format(client):
    ...
    body = client.get(f"/api/v1/plugins/ddw_payment/records/{rid}").json()
    today = datetime.now(timezone.utc).strftime("%Y%m%d")
...
def test_extra_health(client):
    resp = client.get("/api/v1/plugins/ddw_payment/health")
...
def test_extra_filter_by_patient(client):
    client.post(
        "/api/v1/plugins/ddw_payment/records", json=_payload(patient_id="pt_A")
    )
    client.post(
        "/api/v1/plugins/ddw_payment/records", json=_payload(patient_id="pt_B")
    )
    resp = client.get(
        "/api/v1/plugins/ddw_payment/records",
