...
"""ddw_finance_dashboard 测试 conftest：注入 in-memory SQLite + 种子数据。

本插件是聚合查询插件，**不创建新表**，但 conftest 必须显式 import
P1-1 / P1-2 / P1-3 / P1-4 / P0-1 的 ``models``，让它们注册到
``Base.metadata``，否则后续 ``Base.metadata.create_all`` 不会建立
crm_companies / crm_contracts / crm_orders / crm_receivables / crm_offline_pos_records
五张表。

依赖关系（来自 FK 引用）：
- crm_contracts    : 依赖 crm_companies / crm_contacts / crm_opportunities / crm_quotations
- crm_receivables  : 依赖 crm_companies / crm_orders / crm_contracts
- crm_offline_pos_records     : 依赖 crm_companies

因此 conftest 必须同时把 P0-1~P0-4 + P1-1~P1-2 的模型都 import 进来。
"""
...
async def session_factory(engine):
    """构造 sessionmaker，并建表（含 5 个被本插件直接 query 的模型）。"""
    ...
    from plugins.ddw_order import models as ord_models  # noqa: F401  receivable 的 FK
    from plugins.ddw_offline_pos import models as pay_models  # noqa: F401
...
async def service(seeded_db):
    ...
    from plugins.ddw_finance_dashboard.services import DashboardService
