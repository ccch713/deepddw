...
class PaymentStore:
    ...
    def _init_db(self) -> None:
        with self._connect() as conn:
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS offline_pos_records (
                    id TEXT PRIMARY KEY,
            ...
            conn.execute(
                ...
                "ON offline_pos_records(patient_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_payments_date "
                "ON offline_pos_records(created_at)"
            )
    ...
    def create(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if not payload.get("receipt_number") and payload["status"] == "pending":
            payload["receipt_number"] = self._next_receipt_number(now[:10].replace("-", ""))
        with self._lock, self._connect() as conn:
            conn.execute(
                """
                INSERT INTO offline_pos_records (
    ...
    def get(self, record_id: str) -> Optional[dict[str, Any]]:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT * FROM offline_pos_records WHERE id=?", (record_id,)
            ).fetchone()
    ...
    def list_records(
        ...
        with self._connect() as conn:
            total = conn.execute(
                f"SELECT COUNT(*) FROM offline_pos_records {where_sql}", tuple(params)
            ).fetchone()[0]
            rows = conn.execute(
                f"SELECT * FROM offline_pos_records {where_sql} "
    ...
    def update(self, record_id: str, updates: dict[str, Any]) -> Optional[dict[str, Any]]:
        ...
        with self._lock, self._connect() as conn:
            conn.execute(
                f"UPDATE offline_pos_records SET {fields} WHERE id=?", tuple(values)
            )
        return self.get(record_id)
    ...
    def daily_summary(self, date: str) -> dict[str, Any]:
        ...
        for r in rows:
            if r["status"] == "paid":
                m = r["payment_method"]
                income_by_method[m] = income_by_method.get(m, 0.0) + float(r["actual_amount"])
    ...
    def total_count(self) -> int:
        with self._connect() as conn:
            return int(conn.execute("SELECT COUNT(*) FROM offline_pos_records").fetchone()[0])

    def _next_receipt_number(self, ymd: str) -> str:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT COUNT(*) FROM offline_pos_records WHERE receipt_number LIKE ?",
                (f"R{ymd}%",),
            ).fetchone()
        seq = int(row[0]) + 1
