"""DDW 口腔诊所客服 — 轻量知识库（纯 stdlib 混合检索）.
...
class KnowledgeBase:
    ...
    def __init__(
        self, knowledge_dir: str, chunk_size: int = 500,
        overlap: int = 50,
    ):
    ...
    def _load_all(self) -> None:
        if not self.knowledge_dir.is_dir():
            logger.warning(
                "Knowledge dir not found: %s", self.knowledge_dir,
            )
            return
        for fp in sorted(self.knowledge_dir.rglob("*")):
            if not fp.is_file():
                continue
            if fp.suffix.lower() not in _SUPPORTED_EXTS:
                continue
            try:
            ...
            for para in self._chunk(text):
                self.chunks.append({
                    "content": para,
                    "source": str(
                        fp.relative_to(self.knowledge_dir)
                    ),
                    "vector": self._embed(para),
                })
        logger.info(
            "KB loaded %d chunks from %s",
            len(self.chunks), self.knowledge_dir,
        )
    ...
    def _embed(self, text: str, dim: int = 128) -> List[float]:
        ...
        for tok in self._tokenize(text)[:4000]:
            idx = int(
                hashlib.md5(tok.encode()).hexdigest(), 16
            ) % dim
            vec[idx] += 1.0
