...
"""ddw_partner_directory 测试 conftest：注入 in-memory SQLite + 种子租户 + 占位 crm_companies。

关键：
- 模块级 import 触发 Partner 注册到 Base.metadata
...
from plugins.ddw_partner_directory import models as _partner_models  # noqa: E402, F401
...
# 便捷：直接构造 PartnerService（用于单元测试）
@pytest_asyncio.fixture
async def service(seeded_db):
    from plugins.ddw_partner_directory.services import PartnerService

    return PartnerService(seeded_db)
