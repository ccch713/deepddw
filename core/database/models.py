...
class User(Base, TenantMixin, TimestampMixin):
    ...
    # 用户管理改版（G 项）：用户分类 / 子管理员频道权限 / 批量停用
    user_type: Mapped[str] = mapped_column(String(20), default="saas", nullable=False)  # demo / dealer / saas / onpremise
    channel_perms: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)  # 子管理员频道权限；None=全部
    disabled_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)  # 停用时间
...
# ---------------------------------------------------------------------------
# 用户管理改版（G 项：角色管理 + 独立部署客户档案 + 授权码 + 插件单价）
# ---------------------------------------------------------------------------
...
class Role(Base, TimestampMixin):
    ...
    channel_perms: Mapped[Optional[list]] = mapped_column(JSON, nullable=False, default=list)  # 可见频道清单；空=全部
    is_system: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)  # 系统内置不可删
    created_by: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)


class PluginMeta(Base):
    """插件元数据扩展（单价等，供授权码更新差价计算）。"""

    __tablename__ = "plugin_meta"
    __table_args__ = ({"extend_existing": True},)

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    plugin_name: Mapped[str] = mapped_column(String(120), unique=True, nullable=False, index=True)
    price_cny: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)  # 插件单价（元）


class OnPremiseCustomer(Base, TimestampMixin):
    """独立部署客户档案。"""
    ...
    first_license_date: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
...
class LicenseKey(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    customer_id: Mapped[int] = mapped_column(Integer, ForeignKey("onpremise_customers.id", ondelete="CASCADE"), nullable=False, index=True)
    license_code: Mapped[str] = mapped_column(String(120), unique=True, nullable=False)
...
class LicensePluginChange(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    license_id: Mapped[int] = mapped_column(Integer, ForeignKey("license_keys.id", ondelete="CASCADE"), nullable=False, index=True)
    action: Mapped[str] = mapped_column(String(10), nullable=False)  # add / remove
    plugin_name: Mapped[str] = mapped_column(String(120), nullable=False)
...
class PluginMarketItem(Base):
    """插件市场元数据（中文名/分类/安装次数/星级/更新日期/价格）。"""

    __tablename__ = "plugin_market_items"
    __table_args__ = ({"extend_existing": True},)

    plugin_name: Mapped[str] = mapped_column(String(100), primary_key=True)
    title: Mapped[str] = mapped_column(String(200), default="", nullable=False)
    category: Mapped[str] = mapped_column(String(50), default="通用", nullable=False)
    installs: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    stars: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    star_count: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    updated_at: Mapped[str] = mapped_column(String(20), default="", nullable=False)
    # 定价试点（2026-08-14 用户拍板）：price_cny 单位=分；0 表示不公开/不挂价
    price_cny: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
    price_note: Mapped[str] = mapped_column(String(200), default="", nullable=False)
...
class PluginStar(Base):
    """插件评分（唯一约束 plugin_name + user_id，支持 upsert）。"""

    __tablename__ = "plugin_stars"
    __table_args__ = (
        UniqueConstraint("plugin_name", "user_id", name="uq_plugin_star_user"),
        {"extend_existing": True},
    )
...
class IMAuditLog(Base):
    ...
    id: Mapped[int] = mapped_column(BigInt, primary_key=True, autoincrement=True)
    platform: Mapped[str] = mapped_column(String(20), nullable=False, index=True)  # dingtalk/feishu/wecom
