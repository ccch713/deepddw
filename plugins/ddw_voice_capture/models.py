...
class VoiceRecord(Base, TenantMixin, TimestampMixin):
    ...
    # mobile_app / web / phone / import / meeting_room

    # ---- 备注 / 状态 ----
    notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    status: Mapped[str] = mapped_column(
        String(20), default="uploaded", nullable=False, index=True
    )  # uploaded / transcribing / transcribed / failed / archived

    # ---- 审计 ----
    created_by: Mapped[Optional[int]] = mapped_column(BigInt, nullable=True)

    def __repr__(self) -> str:  # pragma: no cover
        return f"<VoiceRecord id={self.id} source={self.source_type} status={self.status}>"
