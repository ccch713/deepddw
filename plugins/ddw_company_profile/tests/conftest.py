...
"""ddw_company_profile 测试 conftest：注入 in-memory SQLite + 种子数据。"""
...
async def session_factory(engine):
    ...
    from plugins.ddw_company_profile import models as plugin_models  # noqa: F401
    # 注册依赖本插件的其它插件（crm_companies 被 crm_contacts/crm_opportunities 引用）
    # SQLAlchemy create_all 要求 FK 目标表都在 metadata
    try:
...
# 便捷：直接构造 CompanyService（用于单元测试）
@pytest_asyncio.fixture
async def service(seeded_db):
    from plugins.ddw_company_profile.services import CompanyService

    return CompanyService(seeded_db)
